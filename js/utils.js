...
NexT.utils = {
  ...
  registerFoldCode: function() {
    document.querySelectorAll('figure.highlight').forEach(figure => {

      const foldButton = document.createElement('button');
      foldButton.innerHTML = '&#x25bd;'; // 下三角形字符
      foldButton.className = 'fold-button';
      foldButton.style.cursor = 'pointer';

      const figcaption = figure.querySelector('figcaption');
      if (figcaption) {
        figcaption.insertBefore(foldButton, figcaption.firstChild);

        foldButton.addEventListener('click', () => {
          const codeBlock = figure.querySelector('.code-container') || figure.querySelector('.highlight');
          codeBlock.style.display = codeBlock.style.display === 'none' ? '' : 'none';
          foldButton.innerHTML = codeBlock.style.display === 'none' ? '&#x25b7;' : '&#x25bd;'; // 变换三角形方向
        });
      }
    });
  },
  ...
  registerCodeblock: function(element) {
    ...
    figure.forEach(element => {
      ...
      if(needFold){
        const container = element.querySelector('.code-container') || element;
        if(container){

          container.style.display = 'none';
        }
        const foldButton = element.querySelector('.fold-button') || element;
        if(foldButton){
          console.log('Processing figure with classes:', foldButton.className, 'and id:', foldButton.id);
